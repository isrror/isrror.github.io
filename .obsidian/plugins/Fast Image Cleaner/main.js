...
var deleteAttach = (file, plugin) => __async(void 0, null, function* () {
  ...
  try {
    ...
    console.error(error);
    new import_obsidian3.Notice("Faild to delete the attachment !", SUCCESS_NOTICE_TIMEOUT2);
  }
});
var deleteNote = (file) => {
    ...
    if ((activeMd == null ? void 0 : activeMd.path) === mdFile) {
      ...
      for (const [filePath, nr] of Object.entries(links)) {
        ...
        try {
          ...
          if (!fileCount) {
            yield app.vault.delete(parentFolder, true);
            new import_obsidian4.Notice("All attachments with parent folder deleted!", 3e3);
          }
        } catch (error) {
          console.warn(error);
        }
